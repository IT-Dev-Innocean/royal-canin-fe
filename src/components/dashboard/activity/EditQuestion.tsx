...
import type { EventActivityQuestion } from './types';
...
function formFromQuestion(q: EventActivityQuestion) {
  return {
    body: q.body ?? '',
    reward_points:
      q.reward_points != null && !Number.isNaN(Number(q.reward_points))
        ? String(q.reward_points)
        : '0',
    sort_order: String(q.sort_order ?? 1),
...
export interface EditQuestionProps {
  open: boolean;
  activityId: number;
  question: EventActivityQuestion | null;
  onClose: () => void;
  onSuccess?: () => void;
  onToast?: (type: 'success' | 'error', message: string) => void;
}

type Form = ReturnType<typeof formFromQuestion>;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `/api/admin/event-activities/${activityId}/questions/${question.id}`,
        {
          method: 'PUT',
          headers: {
      ...
      if (!res.ok || json.success === false) {
        ...
        onToast?.('error', json.message ?? 'Gagal memperbarui pertanyaan.');
        return;
      }
      onSuccess?.();
      onClose();
      onToast?.('success', json.message ?? 'Pertanyaan berhasil diperbarui.');
    } catch {
    ...
    <div className='fixed inset-0 z-120 flex items-end justify-center sm:items-center'>
      ...
      <div className='relative z-10 flex max-h-[min(92vh,700px)] w-full max-w-lg flex-col rounded-t-2xl border border-gray-100 bg-white shadow-2xl sm:rounded-2xl'>
        <div className='flex items-center justify-between border-b border-gray-100 px-5 py-4'>
          <h3 className='text-lg font-bold text-gray-900'>Ubah Pertanyaan</h3>
          <button
            type='button'
            disabled={saving}
            onClick={handleClose}
