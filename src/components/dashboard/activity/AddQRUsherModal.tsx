...
export interface AddQRUsherModalProps {
  open: boolean;
  activityId: number;
  /** Diisi dari default_reward_points aktivitas bila tersedia. */
...
export function AddQRUsherModal({
  open,
  activityId,
  defaultRewardPoints,
  onClose,
  ...
  useEffect(() => {
    if (open) {
      setPublicToken(generatePublicToken());
      setFieldErrors({});
      if (
  ...
  }, [open, activityId, defaultRewardPoints]);
  ...
  function handleRegenerate() {
    if (!saving) setPublicToken(generatePublicToken());
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `/api/admin/event-activities/${activityId}/scannable-codes`,
        {
          method: 'POST',
          headers: {
      ...
      if (!res.ok || json.success === false) {
        ...
        onToast?.('error', json.message ?? 'Gagal membuat QR usher reward.');
        return;
      }
      onSuccess?.();
      onClose();
      onToast?.('success', json.message ?? 'QR token usher reward berhasil dibuat.');
    } catch {
    ...
    <div className='fixed inset-0 z-120 flex items-end justify-center sm:items-center'>
      ...
      <div className='relative z-10 flex max-h-[min(92vh,700px)] w-full max-w-lg flex-col rounded-t-2xl border border-gray-100 bg-white shadow-2xl sm:rounded-2xl'>
        <div className='flex items-center justify-between border-b border-gray-100 px-5 py-4'>
          <h3 className='text-lg font-bold text-gray-900'>Add QR Usher</h3>
          <button
            type='button'
            disabled={saving}
            onClick={handleClose}
        ...
        <form
          ...
          <div className='min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4'>
            <div>
              <label
                htmlFor='add-usher-code-kind'
                className='text-xs font-bold text-gray-600'>
                code_kind
              </label>
              <input
                id='add-usher-code-kind'
                type='text'
                value={CODE_KIND}
                readOnly
                tabIndex={-1}
