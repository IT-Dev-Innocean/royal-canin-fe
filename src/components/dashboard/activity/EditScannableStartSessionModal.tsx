...
export interface EditScannableStartSessionModalProps {
  open: boolean;
  activityId: number;
  /** Kode bertipe `start_session` untuk di-edit. */
...
export function EditScannableStartSessionModal({
  open,
  activityId,
  code,
  onClose,
  onSuccess,
  onToast,
}: EditScannableStartSessionModalProps) {
  ...
  function handleRegenerate() {
    ...
    if (!saving) setPublicToken(`DEMO-START-${suffix}`);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || json.success === false) {
        ...
        onToast?.('error', json.message ?? 'Gagal memperbarui start session.');
        return;
      }
      onSuccess?.();
      onClose();
      onToast?.(
        'success',
        json.message ?? 'Kode start session berhasil diperbarui.',
      );
    } catch {
    ...
    <div className='fixed inset-0 z-120 flex items-end justify-center sm:items-center'>
      ...
      <div className='relative z-10 flex max-h-[min(92vh,700px)] w-full max-w-lg flex-col rounded-t-2xl border border-gray-100 bg-white shadow-2xl sm:rounded-2xl'>
        <div className='flex items-center justify-between border-b border-gray-100 px-5 py-4'>
          <h3 className='text-lg font-bold text-gray-900'>Ubah Session</h3>
          <button
            type='button'
            disabled={saving}
            onClick={handleClose}
        ...
        <form
          ...
          <div className='min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4'>
            <div>
              <label
                htmlFor='edit-ss-code-kind'
                className='text-xs font-bold text-gray-600'>
                code_kind
              </label>
              <input
                id='edit-ss-code-kind'
                type='text'
                value={CODE_KIND}
                readOnly
                tabIndex={-1}
