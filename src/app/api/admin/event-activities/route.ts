...
const EVENT_ACTIVITIES_URL = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/admin/activities`;
...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = new URL(EVENT_ACTIVITIES_URL);
      ...
      return NextResponse.json(
        {
          ...
          message:
            ...
            "Gagal mengambil daftar aktivitas.",
        },
...
export async function POST(request: NextRequest) {
  ...
  try {
    const apiRes = await fetch(EVENT_ACTIVITIES_URL, {
      method: "POST",
      headers: {
