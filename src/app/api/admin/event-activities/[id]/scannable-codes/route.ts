...
const scannableUrl = (id: string) =>
  `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/admin/activities/${id}/scannable-codes`;
...
export async function POST(
  ...
  try {
    const apiRes = await fetch(scannableUrl(id), {
      method: "POST",
      headers: {
    ...
    if (Object.keys(apiData).length === 0) {
      return NextResponse.json(
        { success: true, message: "Kode scannable berhasil dibuat." },
