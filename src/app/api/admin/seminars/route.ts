...
const SEMINARS_URL = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/admin/seminars`;
...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = new URL(SEMINARS_URL);
  ...
  try {
    ...
    if (!apiRes.ok) {
      return NextResponse.json(
        {
          success: false,
          message: apiData.message ?? "Gagal mengambil daftar seminar.",
        },
...
export async function POST(request: NextRequest) {
  ...
  try {
    const apiRes = await fetch(SEMINARS_URL, {
      method: "POST",
      headers: {
        Accept: "application/json",
        Authorization: authHeader,
    ...
    if (!apiRes.ok) {
      return NextResponse.json(
        {
          success: false,
          message: apiData.message ?? "Gagal menambahkan seminar.",
