...
const BASE = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/admin/seminars`;
  ...
  try {
    ...
    if (!apiRes.ok) {
      return NextResponse.json(
        {
          success: false,
          message: apiData.message ?? "Gagal mengambil detail seminar.",
        },
  ...
  try {
    ...
    if (!apiRes.ok) {
      return NextResponse.json(
        {
          success: false,
          message: apiData.message ?? "Gagal memperbarui seminar.",
