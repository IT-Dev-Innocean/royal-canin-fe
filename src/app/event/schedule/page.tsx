  ...
  return (
    <main className='flex flex-col items-center p-6 min-h-screen text-black'>
      ...
      <div className='mb-10 text-center'>
        <h1 className='text-xl md:text-2xl font-bold mt-0'>Agenda Acara</h1>
      </div>
      ...
      <div className='w-full max-w-lg relative px-2'>
        ...
        {agendaData.map((item, index) => (
          <div key={index} className='relative pl-8 md:pl-10 mb-2 w-full'>
            <div className='absolute left-0 top-1 w-4 h-4 bg-rc-red rounded-full z-10 border-4 border-white shadow-sm'></div>
            ...
            <div className='bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-4 border border-gray-100 mb-6 transition-all hover:shadow-md'>
              <h3
                className={`font-bold text-[14px] leading-tight ${item.isItalic ? 'italic text-gray-600' : 'text-gray-900'}`}>
                {item.title}
              </h3>
              {item.speaker && (
                <p className='text-[12px] font-bold italic text-rc-red mt-1 leading-snug'>
                  {item.speaker}
                </p>
              )}
      ...
      <div className='w-full max-w-md mt-6 pb-20'>
        <Link
          href='/event'
          className='block w-full py-3 bg-rc-red text-white text-center rounded-full font-bold shadow-lg transition-all hover:bg-rc-red/80 active:scale-95'>
          Kembali
        </Link>
      </div>
    </main>
